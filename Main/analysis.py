...
def summing():
    array = []
    os.chdir("/Users/Euan/Desktop/NCC2019/Main")
...
def distanceBetweenTwos():
    ...
    for n in x:
        if n == "2":
            zeroArray.append(zeroCount)
            zeroCount = 0
        else:
            zeroCount+=1
    mean = sum(zeroArray) / len(zeroArray)
    print(mean)

def percentageOfCipherThatIsTwo():
    os.chdir("/Users/Euan/Desktop/NCC2019/Main")
    with open("test.txt", "r") as f:
        x = list(f.read())
        zero  = x.count("0")
